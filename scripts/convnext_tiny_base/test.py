...
def evaluate_tencrop(ema, dataloader, device: torch.device, use_amp: bool) -> tuple[dict, list, list]:
    ...
    for images, labels in pbar:
        # TenCrop: images có shape (B, 10, C, H, W)
...
def main():
    ...
    print("\n" + "=" * 65)
    print(" KẾT QUẢ ĐÁNH GIÁ TRÊN EMA MODEL (ConvNeXt_Tiny backbone) ")
    print("=" * 65)
    ...
    print("-" * 65)
    for name, val in metrics["per_class"].items():
        print(f"  {name.capitalize():<10} | {val['precision'] * 100:8.2f}% | {val['recall'] * 100:8.2f}% | {val['f1'] * 100:8.2f}% | {val['support']:<8}")
    print("=" * 65)

    # Confusion Matrix
    cm_path = out_dir / "confusion_matrix_base_ema.png"
    plot_and_save_confusion_matrix(
        y_true=y_true,
        y_pred=y_pred,
        out_path=cm_path,
        title="A0 Confusion Matrix (ConvNeXt-Tiny Base - EMA TenCrop)",
    )
