...
@torch.no_grad()
def evaluate_tencrop_a2(ema, dataloader, device: torch.device, use_amp: bool) -> tuple[dict, list, list]:
    ema.eval()
    amp_enabled = use_amp and device.type == "cuda"
    all_targets, all_predictions = [], []

    pbar = tqdm(dataloader, desc="Evaluating EMA Model (TenCrop - A2)")
...
def main():
    parser = argparse.ArgumentParser(description="Ablation A2 Test Evaluation")
    parser.add_argument("--checkpoint", type=str, default="checkpoints/convnext_tiny_a2/best.pt", help="Path to checkpoint")
    parser.add_argument("--config", type=str, default="configs/A2/convnext_tiny_a2.yaml", help="Path to A2 config")
    ...
    metrics, y_true, y_pred = evaluate_tencrop_a2(
        ema=ema,
    ...
    print("\n" + "=" * 65)
    print(" KẾT QUẢ ĐÁNH GIÁ TRÊN EMA MODEL (A2) ")
    print("=" * 65)
    ...
    plot_and_save_confusion_matrix(
        y_true=y_true,
        y_pred=y_pred,
        out_path=out_dir / "confusion_matrix_a2_ema.png",
        title="A2 Confusion Matrix (6 Regions + Fusion - TenCrop)",
    )
