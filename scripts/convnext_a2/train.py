...
def _train_one_epoch_a2(
    ...
    for batch_idx, (images, labels, heatmaps_P, valid_masks) in enumerate(pbar):
        ...
        heatmaps_P = heatmaps_P.to(device)       # (B, 6, 14, 14)
        valid_masks = valid_masks.to(device)     # (B, 6)
...
def _evaluate_one_epoch_a2(
    ...
    pbar = tqdm(dataloader, desc="Validation A2")
...
def _append_epoch_log_a2(path: Path, epoch: int, lr: float, train_metrics: dict, val_metrics: dict) -> None:
...
def main() -> None:
    ...
    if not landmarks_path.exists():
        raise FileNotFoundError(
            f"[ERROR] Landmark PKL không tìm thấy tại: {landmarks_path}\n"
            "Hãy đảm bảo file fer2013_landmarks.pkl đã được đặt trong folder data/."
        )
    with open(landmarks_path, "rb") as f:
        landmarks_dict = pickle.load(f)
    print(f"[A2] Loaded landmarks cho {len(landmarks_dict):,} images.")

    # Prior Generator 6 regions
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        _append_epoch_log_a2(output_dir / "history_a2.csv", epoch, current_lr, train_metrics, val_metrics)
    ...
    print(f"Best checkpoint saved at: {(checkpoint_dir / 'best.pt').resolve()}")
